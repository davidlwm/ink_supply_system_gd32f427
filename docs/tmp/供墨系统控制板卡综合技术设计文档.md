...
│   │
│   ├── drivers/                           # 底层驱动模块 (BSP层)
│   │   ├── gd32f427_hal/                  # GD32F427 HAL库
│   │   ├── gpio_driver.c/h                # GPIO驱动
│   │   ├── adc_driver.c/h                 # ADC驱动
│   │   ├── dac_driver.c/h                 # DAC驱动
│   │   ├── pwm_driver.c/h                 # PWM驱动
│   │   ├── uart_driver.c/h                # UART驱动
│   │   ├── spi_driver.c/h                 # SPI驱动
│   │   ├── i2c_driver.c/h                 # I2C驱动
│   │   └── timer_driver.c/h               # 定时器驱动
│   │
│   ├── hal/                               # 硬件抽象层
│   │   ├── sensor_hal.c/h                 # 传感器硬件抽象
│   │   ├── actuator_hal.c/h               # 执行器硬件抽象
│   │   ├── communication_hal.c/h          # 通信硬件抽象
│   │   └── system_hal.c/h                 # 系统硬件抽象
│   │
│   ├── sensors/                           # 传感器管理模块
│   │   ├── liquid_level_sensor.c/h        # 液位传感器 (FRD-8061)
│   │   ├── pressure_sensor.c/h            # 压力传感器 (HP10MY)
│   │   ├── temperature_sensor.c/h         # 温度传感器 (FTT518)
│   │   ├── adc_manager.c/h                # ADC采集管理
│   │   ├── signal_processing.c/h          # 信号处理算法
│   │   └── sensor_calibration.c/h         # 传感器校准
│   │
│   ├── actuators/                         # 执行器控制模块
│   │   ├── heater_control.c/h             # 加热器控制
│   │   ├── pump_control.c/h               # 泵控制
│   │   ├── valve_control.c/h              # 电磁阀控制
│   │   ├── pwm_output.c/h                 # PWM输出控制
│   │   └── actuator_protection.c/h        # 执行器保护
│   │
│   ├── control/                           # 控制算法模块
│   │   ├── pid_controller.c/h             # PID控制器
│   │   ├── temperature_control.c/h        # 温度控制
│   │   ├── pressure_control.c/h           # 压力控制
│   │   ├── liquid_level_control.c/h       # 液位控制
│   │   ├── flow_control.c/h               # 流量控制
│   │   └── adaptive_control.c/h           # 自适应控制
│   │
│   ├── communication/                     # 通信模块
│   │   ├── ethercat/                      # EtherCAT通信
│   │   │   ├── ethercat_driver.c/h        # EtherCAT驱动
│   │   │   ├── ethercat_protocol.c/h      # EtherCAT协议
│   │   │   └── ethercat_config.c/h        # EtherCAT配置
│   │   ├── tcp_ip/                        # TCP/IP通信
│   │   │   ├── ethernet_driver.c/h        # 以太网驱动 (YT8512C)
│   │   │   ├── tcp_server.c/h             # TCP服务器
│   │   │   ├── modbus_tcp.c/h             # Modbus TCP协议
│   │   │   └── network_config.c/h         # 网络配置
│   │   ├── serial/                        # 串行通信
│   │   │   ├── rs485_driver.c/h           # RS485驱动
│   │   │   ├── rs232_driver.c/h           # RS232驱动
│   │   │   └── protocol_handler.c/h       # 协议处理
│   │   └── communication_manager.c/h      # 通信管理
│   │
│   ├── hmi/                               # 人机界面模块
│   │   ├── led_manager.c/h                # LED指示灯管理
│   │   ├── display_manager.c/h            # LCD显示管理 (CH12832B-12)
│   │   ├── display_pages.c/h              # 显示页面管理
│   │   ├── key_handler.c/h                # 按键处理 (如果有)
│   │   ├── menu_system.c/h                # 菜单系统
│   │   └── hmi_main.c/h                   # HMI主控制
│   │
│   ├── monitor/                           # 系统监控模块
│   │   ├── health_monitor.c/h             # 系统健康监控
│   │   ├── performance_monitor.c/h        # 性能监控
│   │   ├── fault_detection.c/h            # 故障检测
│   │   ├── fault_diagnosis.c/h            # 故障诊断
│   │   ├── alarm_manager.c/h              # 报警管理
│   │   └── diagnostics.c/h                # 系统诊断
│   │
│   ├── config/                            # 配置管理模块
│   │   ├── config_manager.c/h             # 配置管理
│   │   ├── flash_storage.c/h              # Flash存储
│   │   ├── eeprom_storage.c/h             # EEPROM存储
│   │   ├── parameter_validation.c/h       # 参数验证
│   │   ├── config_defaults.c/h            # 默认配置
│   │   └── config_backup.c/h              # 配置备份
│   │
│   ├── logging/                           # 日志记录模块
│   │   ├── log_manager.c/h                # 日志管理
│   │   ├── log_buffer.c/h                 # 日志缓冲
│   │   ├── log_storage.c/h                # 日志存储
│   │   ├── log_format.c/h                 # 日志格式化
│   │   └── log_output.c/h                 # 日志输出
│   │
│   ├── safety/                            # 安全保护模块
...
│       ├── data_filter.c/h                # 数据滤波
│       └── conversion_utils.c/h           # 数据转换
│
├── inc/                                   # 头文件目录
│   ├── main.h                             # 主头文件
...
│
├── tools/                                 # 开发工具
│   ├── flash_programmer/                  # 烧录工具
│   ├── configuration_tool/                # 配置工具
│   └── debugging_scripts/                 # 调试脚本
│
├── build/                                 # 构建输出目录
│   ├── obj/                               # 目标文件
│   ├── bin/                               # 二进制文件
